...
public:
    ...
    // getter / setter methods are use to access the private variables
    string getGfName()
    {
        return gf;
        // return this->gf;
...
int main()
{
    ...
    // allocating on stack memory
    // Student s3(2, 15, 0, "Lokesh", 5);
    // cout << s3.name << endl;

    // // alocating on heap memory
    Student *s4 = new Student(3, 34, 1, "Chintu", 6);
    // use arrow to access the member of object if object created on heap while dot for stack memory
    cout << s4->name << endl;
    // to access the member from heap object using dot the first derefer it by pointer
    cout << (*s4).name << endl;
